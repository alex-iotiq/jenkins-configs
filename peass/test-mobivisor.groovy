pipeline {
    ...
    stages {
        ...
        stage('filter passing tests'){
            steps{
                script{
                    sh 'git -c sequence.editor="sed -i \'\'" rebase -i "HEAD~3" --exec "cp -fr ${JENKINS_HOME}/peass_scripts/build.gradle ${WORKSPACE}/app && python3 ${JENKINS_HOME}/peass_scripts/find_unit_tests.py ${WORKSPACE}/app/src && git add ${WORKSPACE}/app/build.gradle ${WORKSPACE}/app/src/test/* && git commit --amend --no-edit --allow-empty" -X "theirs"'
